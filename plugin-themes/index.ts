...
import { funcCollectPlugin } from './plugins/postcss/func-collect-plugin';
import { declVarPlugin } from './plugins/postcss/dec-var-plugin';
import { parseThemesData, setThemesData, getThemesData, getThemesDataStr } from './utils/themes';
...
const plugin: IPlugin = async (api, options = {}) => {
  ...
  onGetWebpackConfig(config => {
     ...
     * getCode 会在相应阶段执行
     */
    config.plugin('define-variable-plugin').use(AdvanceDefinedPlugin, [
      {
        getCode: () => {
          // get data form cache
